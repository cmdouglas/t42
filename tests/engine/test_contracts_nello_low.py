...
def test_doubles_rank_low_in_their_number_suit_regardless_of_the_table_config() -> None:
    ...
    # number suit and ranks them lowest there - intrinsic to the contract, not RuleConfig-driven.
    on_config = RuleConfig(doubles_are_own_suit=True)
    trick = Trick(
        plays=(
...
def test_a_non_double_still_beats_nothing_when_alone_but_loses_to_a_higher_non_double() -> None:
    ...
    assert NELLO_LOW.trick_winner(trick, None, RuleConfig()) is Seat.EAST
